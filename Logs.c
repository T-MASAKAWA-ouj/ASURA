...
void InitLogFiles(void){
    ...
    sprintf(FnameEnergy,"Energy.data");
    sprintf(FnameMomentum,"Momentum.data");
    sprintf(FnameAMomentum,"AMomentum.data");
    sprintf(FnameElapsedTime,"ElapsedTime.data");
...
void LogOutPutEnergyMomentumAngularMomentum(void){
    ...
    } LocalPhysicalQuantities,GlobalPhysicalQuantities;

    memset(&LocalPhysicalQuantities,0,sizeof(struct LogPhysicalQuantities));

    // calc local
    for(int i=0;i<Pall.Ntotal;i++){
        LocalPhysicalQuantities.Ek += Pbody[i]->Mass*NORM2(Pbody[i]->Vel);
        LocalPhysicalQuantities.Ep += Pbody[i]->Pot;
        if(Pbody[i]->Type == TypeHydro){
            LocalPhysicalQuantities.Ethermal += Pbody[i]->Mass*PbodyHydroU(i);
        }

        LocalPhysicalQuantities.Momentum[0] += Pbody[i]->Mass*Pbody[i]->Vel[0];
        LocalPhysicalQuantities.Momentum[1] += Pbody[i]->Mass*Pbody[i]->Vel[1];
        LocalPhysicalQuantities.Momentum[2] += Pbody[i]->Mass*Pbody[i]->Vel[2];

        LocalPhysicalQuantities.AMomentum[0] += 
            Pbody[i]->Mass*(Pbody[i]->Pos[1]*Pbody[i]->Vel[2]-Pbody[i]->Pos[2]*Pbody[i]->Vel[1]);
        LocalPhysicalQuantities.AMomentum[1] += 
            Pbody[i]->Mass*(Pbody[i]->Pos[2]*Pbody[i]->Vel[0]-Pbody[i]->Pos[0]*Pbody[i]->Vel[2]);
        LocalPhysicalQuantities.AMomentum[2] += 
            Pbody[i]->Mass*(Pbody[i]->Pos[0]*Pbody[i]->Vel[1]-Pbody[i]->Pos[1]*Pbody[i]->Vel[0]);

        LocalPhysicalQuantities.Mass += Pbody[i]->Mass;
    }
    LocalPhysicalQuantities.Eloss = Pall.CoolingEnergyLoss;

    // communication
    MPI_Reduce(&LocalPhysicalQuantities,&GlobalPhysicalQuantities,11,MPI_DOUBLE,MPI_SUM,0,MPI_COMM_WORLD);

    // IO
    if(MPIGetMyID() == MPI_ROOT_RANK){
        fprintf(FpEnergy,"%e %e %e %e %e %e %e\n",Pall.TCurrent,Pall.Redshift,
                0.5*GlobalPhysicalQuantities.Ek,
                GlobalPhysicalQuantities.Ep,
                GlobalPhysicalQuantities.Ethermal,
                GlobalPhysicalQuantities.Eloss,
                0.5*GlobalPhysicalQuantities.Ek+GlobalPhysicalQuantities.Ep
                 +GlobalPhysicalQuantities.Ethermal+GlobalPhysicalQuantities.Eloss);

        fprintf(FpMomentum,"%e %e %e %e %e %e %e %e\n",Pall.TCurrent,Pall.Redshift,
                GlobalPhysicalQuantities.Momentum[0],GlobalPhysicalQuantities.Momentum[1],
                GlobalPhysicalQuantities.Momentum[2],
                GlobalPhysicalQuantities.Momentum[0]/GlobalPhysicalQuantities.Mass,
                    GlobalPhysicalQuantities.Momentum[1]/GlobalPhysicalQuantities.Mass,
                        GlobalPhysicalQuantities.Momentum[2]/GlobalPhysicalQuantities.Mass);

        fprintf(FpAMomentum,"%e %e %e %e %e %e %e %e\n",Pall.TCurrent,Pall.Redshift,
                GlobalPhysicalQuantities.AMomentum[0],GlobalPhysicalQuantities.AMomentum[1],
                GlobalPhysicalQuantities.AMomentum[2],
                GlobalPhysicalQuantities.AMomentum[0]/GlobalPhysicalQuantities.Mass,
                    GlobalPhysicalQuantities.AMomentum[1]/GlobalPhysicalQuantities.Mass,
                        GlobalPhysicalQuantities.AMomentum[2]/GlobalPhysicalQuantities.Mass);
    }
...
void LogOutPutEnergyMomentumAngularMomentumPredictors(void){
    ...
    for(int i=0;i<Pall.Ntotal;i++){
        ...
        LocalPhysicalQuantities.Ep += Pbody[i]->Pot;
        if(Pbody[i]->Type == TypeHydro)
            LocalPhysicalQuantities.Ethermal += Pbody[i]->Mass*PbodyHydroUPred(i);
