...
#ifdef TASK_dSphs
int main_dSphsModel(const int argc, char *argv[]);
#endif //TASK_dSphs
...
#endif
#ifdef TASK_dSphs
    return main_dSphsModel(argc,argv);
#endif
...
int main_ColdCollapseTest_ActiveParticles(void){
    ...
    fprintf(stderr,"[%02d] =F= Just after the decomposition = %ld, %Ld\n",
...
int main_MilkywayModel(const int argc, char *argv[]){
    ...
    InitializeCommunicationBuffers();
    GetRunStatus(argc,argv);
    int NParticles = 500000;		// Number of SPH particles
    //int NParticles = 1000000;		// 1,000,000 Lv.1
    //int NParticles = 2000000;		// 2,000,000 Lv.2
    //int NParticles = 4000000;		// 4,000,000 Lv.3
    //int NParticles = 8000000;		// 8,000,000 Lv.4
    if(Pall.RunStatus == NewSimulation){
...
int main_DiceRun(const int argc, char *argv[]){
    ...
    if(Pall.RunStatus == NewSimulation){
        ...
        //ReadDiceBinary("dice.asura_bin");
    } else if (Pall.RunStatus == RestartSimulation){
        ParallelReadAllData();
        Pall.RunStatus = RestartSimulation;

        Pall.TEnd = 10*GIGAYEAR_CGS/Pall.UnitTime;
...
static void WriteStromgrenSphere(const int Z_in_PC){
...
int main_dSphsModel(const int argc, char *argv[]){
    ...
    //printf("Debug");
    if(Pall.RunStatus == NewSimulation){
        InitializeParticleDistributionFordSphs();
    } else if (Pall.RunStatus == RestartSimulation){
        ParallelReadAllData();
        Pall.RunStatus = RestartSimulation;
    } else {
