...
// #endif
    ...
    //double Eps[] = {150.0*PC_CGS/Pall.UnitLength,300.0*PC_CGS/Pall.UnitLength,600.0*PC_CGS/Pall.UnitLength}; //ref 164
    ...
    Pall.OutPutInterval = (Pall.TEnd-Pall.TCurrent)/100.0; 
    //Pall.OutPutInterval = (Pall.TEnd-Pall.TCurrent)/10.0; 
...
void ReadBinaryGalactICSFlag(char fname[]){
    ...
    //double EPS_CDM = 200*PC_CGS/Pall.UnitLength;
    //double EPS_CDM = 50*PC_CGS/Pall.UnitLength;
    //double EPS_CDM = 20*PC_CGS/Pall.UnitLength;
    //double EPS_CDM = 10*PC_CGS/Pall.UnitLength;
    double EPS_CDM = 5*PC_CGS/Pall.UnitLength;
    if(MyID == MPI_ROOT_RANK)
        fprintf(stderr,"The softening length = %g pc\n",EPS_CDM*Pall.UnitLength/PC_CGS);
    ...
    // mycount_halo_gas -> Gas; Cahnge type and add extra information.
#if 1
    // For Milky Way simulations by H.Matsui
    double NumberDensityFeSolar = CELibGetElementNumberDensitySolar(CELibYield_Fe);
    double NumberDensityMgSolar = CELibGetElementNumberDensitySolar(CELibYield_Mg);
    double MgFeSolar = NumberDensityMgSolar / NumberDensityFeSolar;
    double logMgFeSolar = log10(MgFeSolar);
    double WeightFe = CELibGetElementWeight(CELibYield_Fe);
    double WeightMg = CELibGetElementWeight(CELibYield_Mg);
    double InitialMgFe = 0.6;
#endif
    for(int i=0;i<Pall.Nhydro;i++){
        Phydro[i]->Use = ON;
        Phydro[i]->Active = ON;
        ...
        // We here assume the half solar metallicity for the ISM at initially.
#ifdef USE_CELIB //{
        //CELibSetPrimordialMetallicity(PhydroBody(i)->Mass,Phydro[i]->Elements);
        //CELibSetPrimordialMetallicity(Pbody[mycount]->Mass,PbodyHydro(mycount)->Elements);
        //CELibSetSolarMetallicity(Pbody[mycount]->Mass,PbodyHydro(mycount)->Elements);
...
#if 1
	// For Milky Way simulations by H.Matsui
	double NumberDensityFe = Phydro[i]->Elements[CELibYield_Fe] / WeightFe;
	double logNumberDensityFe = log10(NumberDensityFe);
	double NumberDensityMg = logNumberDensityFe + logMgFeSolar + InitialMgFe;
	Phydro[i]->Elements[CELibYield_Mg] = pow(10.0,NumberDensityMg) * WeightMg;
#endif
    ...
    Pall.TEnd = 15*GIGAYEAR_CGS/Pall.UnitTime;
    Pall.TCurrent = 0.e0;
    Pall.Redshift = 0.e0;
    Pall.Era = Pall.EraLocal = 0.e0;
    Pall.TStepTotal = 0;
...
#ifdef TASK_DICE_RUN //{
static void InsertDataDice(const int TargetIndex, float Pos[], float Vel[], float Mass, float Rho, float U,
...
#include "StellarFeedback.h"
        ...
        PbodyStar(TargetIndex)->FormationTime = Age*MEGAYEAR_CGS/Pall.UnitTime;
        ...
        if(Age*MEGAYEAR_CGS > -40.0*MEGAYEAR_CGS){
        //if(Age > -40.0){
            PbodyStar(TargetIndex)->TypeII = false;
            gprintlmpi(Age);
        } else {
            PbodyStar(TargetIndex)->TypeII = true;
            //gprintlmpi(Age*Pall.UnitTime/MEGAYEAR_CGS);
        }
        ...
        if(PbodyStar(TargetIndex)->TypeII == true){
            ...
            gprintlmpi(PbodyStar(TargetIndex)->EventTime);
        }
...
static void ReadDiceLine(char fname[], const int Nlines, const int mycount_comp[], const double Eps[]){
    ...
    for(int i=0;i<Nlines;i++){
        ...
        if(i%NProcs == MyID){
            if(Type == 0){ // hydro
                InsertDataDice(counter_hydro,Pos,Vel,Mass,Rho,U,Age,Metal,TypeHydro,i,Eps[Type]);
                counter_hydro ++;
            } else if(Type == 1){ //DM
                InsertDataDice(counter_dm+mycount_comp[0]+mycount_comp[2]+mycount_comp[3],Pos,Vel,Mass,Rho,U,Age,Metal,TypeDM,i,Eps[Type]);
                counter_dm ++;
            } else {
                InsertDataDice(counter_stars+mycount_comp[0],Pos,Vel,Mass,Rho,U,Age,Metal,TypeStar,i,Eps[Type]);
                counter_stars ++;
            }
            counter_total ++;
        }
...
static void ReadDiceLineBinary(char fname[], const int Nlines, const int mycount_comp[], const double Eps[]){
    ...
    int Ndummy[4];
    double Udummy[4];
    fread(Ndummy,sizeof(int),4,fp);
    fread(Udummy,sizeof(double),4,fp);
...
void ReadDiceASCII(char fname[]){
    ...
    double Eps[] = {50.0*PC_CGS/Pall.UnitLength,25.0*PC_CGS/Pall.UnitLength,25.0*PC_CGS/Pall.UnitLength,25.0*PC_CGS/Pall.UnitLength};
...
void ReadDiceBinary(char fname[]){
    ...
    FileOpen(fp,fname,"r");
    if(MPIGetMyID() == MPI_ROOT_RANK){
        fread(Numbers,sizeof(int),4,fp);
        fread(Units,sizeof(double),4,fp);
    }
    fclose(fp);
    MPI_Bcast(&Numbers,4,MPI_INT,MPI_ROOT_RANK,MPI_COMM_WORLD);
    MPI_Bcast(&Units,4,MPI_DOUBLE,MPI_ROOT_RANK,MPI_COMM_WORLD);
    ...
    for(int i=0;i<NProcs;i++){
        if(MyID == i){
            //ReadDiceLine(fname,Numbers[0]+Numbers[1]+Numbers[2]+Numbers[3],mycount_comp,Eps);
            ReadDiceLineBinary(fname,Numbers[0]+Numbers[1]+Numbers[2]+Numbers[3],mycount_comp,Eps);
        }
        MPI_Barrier(MPI_COMM_WORLD);
    }
    ...
    Pall.OutPutFileNumber = 0;
    Pall.OutPutInterval = Pall.TEnd/5000.0; 
...
#define OmegaBar (52.2) // in km/s/kpc
#define Omega_b ( OmegaBar * (1.0e5 / KPC_CGS) * Pall.UnitTime )
...
#define DiskEdge (1.5)		// Rtrunc / Rgas(7.0kpc)  Saitoh+2008
//#define DiskEdge (3.0)
//#define DiskEdge (5.0/7.0)		// Rtrunc / Rgas(7.0kpc)  Saitoh+2008
//#define DiskEdge (0.5)		// Rtrunc / Rgas
//#define DiskEdge (1.0/3.5)

    double fact = (1-exp(-DiskEdge)*(1+DiskEdge));
    double mass = fact*Mgas/(double)Number;	// mass of a SPH
    gprintlmpi(fact);
    ...
    //epsilon *= 0.1;
    //epsilon = 0.2*PC_CGS/Pall.UnitLength;	// Softening Length

// Set epsilon
    epsilon = 5.0*PC_CGS/Pall.UnitLength;	// 5pc
    //epsilon = 1.0*PC_CGS/Pall.UnitLength;
    //epsilon = 0.2*PC_CGS/Pall.UnitLength;
    ...
    for(int i=0;i<Pall.Ntotal;i++){
        ...
        Pbody[i]->Pos[0] = Pos[0];
        Pbody[i]->Pos[1] = Pos[1];
        Pbody[i]->Pos[2] = Pos[2];	// is adopted by Matsui
        Pbody[i]->Pos[2] = Pos[2];	// is adopted by Matsui
        //Pbody[i]->Pos[2] = 0.e0;	// is removed by Matsui
...
#ifdef USE_CELIB //{
//        CELibSetPrimordialMetallicity(Pbody[i]->Mass,PbodyHydro(i)->Elements);	// removed by Matsui
        //CELibSetSolarMetallicity(Pbody[i]->Mass,PbodyHydro(i)->Elements);	// solar mettalicity is adopted by Matsui
    ...
    Pall.TCurrent = 0.e0;
    MilkyWayPotentialHaloDiskBar();
    //InertialForce();	// Inertial force is not needed here.
    ...
    // set rotational coordinate
    for(int i=0;i<Pall.Ntotal;i++){
        Pbody[i]->Vel[0] += Omega_b*Pbody[i]->Pos[1];
        Pbody[i]->Vel[1] -= Omega_b*Pbody[i]->Pos[0];
    }
    ...
    //Pall.OutPutInterval = 0.002*GIGAYEAR_CGS/Pall.UnitTime; 
...
//#define TimeBarStartMyr (0.0) // in Myr
//#define TimeBarEndMyr (300.0) // in Myr
#define TimeBarStartMyr (200.0) // in Myr
#define TimeBarEndMyr (500.0) // in Myr
#define TimeBarGrowthMyr ((TimeBarEndMyr)-(TimeBarStartMyr))	// in Myr
#define R0Bar_KPC (8.0)	// Orbital Radius of Sun
#define RbBar_KPC (3.5)	// Scale of Bar End

void MilkyWayPotentialHaloDiskBar(){
    ...
    /* Parameters for a bulge */
    double Mbulge = 1.7e+10*MSUN_CGS/Pall.UnitMass;      // Sofue2012
    double Abulge = 0.5*KPC_CGS/Pall.UnitLength;
    // double Abulge = 1.0*KPC_CGS/Pall.UnitLength;
    /* Parameters for a bulge */

    /* Parameters for a Supermassive Black Hole */
    double MSMBH = 4.28e+6*MSUN_CGS/Pall.UnitMass;	// Gillessen+2017
    double EpsSMBH = 1.0*PC_CGS/Pall.UnitLength;	//
    ...
    for(int i=0;i<Pall.Ntotal;i++){
	    ...
	    // Bulge
            double r = NORM(Pbody[i]->PosP);
#if 0	// Hernquist (bulge)
	    double gravfact_bulge = Pall.GravConst*Mbulge/SQ(r+Abulge);
            Pbody[i]->Acc[0] -= gravfact_bulge*Pbody[i]->PosP[0]/r;
            Pbody[i]->Acc[1] -= gravfact_bulge*Pbody[i]->PosP[1]/r;
            Pbody[i]->Acc[2] -= gravfact_bulge*Pbody[i]->PosP[2]/r;
#endif
#if 1	// Plummer (bulge)
	    double gravfact_bulge = Pall.GravConst*Mbulge/CUBE(sqrt(SQ(r)+SQ(Abulge)));
            Pbody[i]->Acc[0] -= gravfact_bulge*Pbody[i]->PosP[0];
            Pbody[i]->Acc[1] -= gravfact_bulge*Pbody[i]->PosP[1];
            Pbody[i]->Acc[2] -= gravfact_bulge*Pbody[i]->PosP[2];
#endif

	// SMBH

            double gravfact_smbh = Pall.GravConst*MSMBH/CUBE(sqrt(SQ(r)+SQ(EpsSMBH)));
            Pbody[i]->Acc[0] -= gravfact_smbh*Pbody[i]->PosP[0];
            Pbody[i]->Acc[1] -= gravfact_smbh*Pbody[i]->PosP[1];
            Pbody[i]->Acc[2] -= gravfact_smbh*Pbody[i]->PosP[2];
    ...
    // Add Bar Potential
    /* 
    double CurrentTimeMyr = Pall.TCurrent * Pall.UnitTime / MEGAYEAR_CGS;
    if(CurrentTimeMyr > TimeBarStartMyr){

    	double Alpha = 0.01;	// Bar Amplitude
    	double v0 = 200.0 * 1.0e5 * Pall.UnitTime / Pall.UnitLength;
    	double RbBar = RbBar_KPC*KPC_CGS/Pall.UnitLength;
    	double R0Bar = R0Bar_KPC*KPC_CGS/Pall.UnitLength;

    	double R0OverRb = R0Bar_KPC/RbBar_KPC;
    	double Beta = Alpha * SQ(v0) * CUBE(R0OverRb) / 3.0;
    	double Gamma = - Alpha * SQ(v0) * CUBE(R0Bar) / 3.0;
    	double RbBar3 = CUBE(RbBar);
    	double R0Bar3 = CUBE(R0Bar);

	double eta;
    	if(CurrentTimeMyr < TimeBarEndMyr){

		double xi = 2.0*(CurrentTimeMyr-TimeBarStartMyr)/TimeBarGrowthMyr - 1.0;
		double xi2 = SQ(xi);
		double xi3 = xi2 * xi;
		double xi5 = xi3 * xi2;
		eta = 3.0*xi5/16.0 - 5.0*xi3/8.0 + 15.0*xi/16.0 + 0.5;
	}else{
		eta = 1.0;
	}
		...
		if(r<RbBar){
               		double r2 = SQ(r);
               		double r4 = SQ(r2);
	       		double x = Pbody[i]->PosP[0];
	       		double y = Pbody[i]->PosP[1];
	       		double z = Pbody[i]->PosP[2];

	       		double A = ( 1.0/(RbBar3*r) + 4.0/r4 ) * ( SQ(x)-SQ(y) );
	       		double B = 2.0*r/RbBar3 - 4.0/r2;

               		Pbody[i]->Acc[0] -= eta * Beta * x * (A + B);
               		Pbody[i]->Acc[1] -= eta * Beta * y * (A - B);
               		Pbody[i]->Acc[2] -= eta * Beta * z * A;
			...
			double A = 5.0 * (SQ(x)-SQ(y)) / r7;
			double B = 2.0/r5;

                	Pbody[i]->Acc[0] -= eta * Gamma * x * (-A + B);
                	Pbody[i]->Acc[1] -= eta * Gamma * y * (-A - B);
                	Pbody[i]->Acc[2] -= eta * Gamma * z * (-A);
...
void InertialForce(){
    ...
    for(int i=0;i<Pall.Ntotal;i++){
        if(Pbody[i]->Active){
	        ...
	        Pbody[i]->Acc[0] += SQ(Omega_b) * Pbody[i]->PosP[0];
	        Pbody[i]->Acc[1] += SQ(Omega_b) * Pbody[i]->PosP[1];
	        Pbody[i]->Acc[2] += 0.0;

                // Inertial Force (Coriolis Force)
	        Pbody[i]->Acc[0] += 2.0 * Omega_b * Pbody[i]->Vel[1];
	        Pbody[i]->Acc[1] += - 2.0 * Omega_b * Pbody[i]->Vel[0];
	        Pbody[i]->Acc[2] += 0.0;
	}
    }
...
void MilkyWayPotentialHaloDisk(){
...
#ifdef USE_CELIB //{
            CELibSetSolarAbundancePattern(Pbody[TargetIndex]->Mass,PbodyHydro(TargetIndex)->Elements);
            double MassLightElements = PbodyHydro(TargetIndex)->Elements[CELibYield_H]
                                      +PbodyHydro(TargetIndex)->Elements[CELibYield_He];
            double Z = (Pbody[TargetIndex]->Mass-MassLightElements)/Pbody[TargetIndex]->Mass;
            PbodyHydro(TargetIndex)->Z = PbodyHydro(TargetIndex)->ZII = PbodyHydro(TargetIndex)->ZIa = Z;
...
#ifdef TASK_dSphs
void InitializeParticleDistributionFordSphs(void){
  ...
  int i,j;
    ...
    // int Number_h, Number_DM;
    
    //Number_h = Number
    fscanf(fp, "%ld%ld%ld", &Pall.Ntotal_t, &Pall.NDM_t, &Pall.Nhydro_t);
    printf("reading initial condtion file\n");
    double Pos[Pall.Ntotal_t][3], Vel[Pall.Ntotal_t][3], Mass[Pall.Ntotal_t];
    
    for(i =0; i < Pall.Ntotal_t; i++){
      fscanf(fp, "%lf%lf%lf%lf%lf%lf%lf", &Pos[i][0],&Pos[i][1],&Pos[i][2],&Vel[i][0],&Vel[i][1],&Vel[i][2],&Mass[i]);
    ...
    int mycount_body_t = 0;
    for(int k=0;k<Pall.Ntotal_t;k+=NProcs)
        mycount_body_t ++;
    
    int mycount_body_DM = 0;
    for(int k=0;k<Pall.NDM_t;k+=NProcs)
        mycount_body_DM ++;

    int mycount_body_hydro = 0;
    for(int k=0;k<Pall.Nhydro_t;k+=NProcs)
        mycount_body_hydro ++;

    Pall.Ntotal = mycount_body_t;
    Pall.NDM = mycount_body_DM;
    Pall.Nhydro = mycount_body_hydro;

    //    Pall.Ntotal_t = Number;
    // Pall.NDM_t = Number1;
    // Pall.Nhydro_t = Number1;

    PbodySize = Pall.Ntotal_t;
    PhydroSize = Pall.Nhydro_t;
    ...
    for(i=0;i<PhydroSize;i++){
        Pbody[i]->Baryon = (void *)(Phydro[i]);
        Phydro[i]->Body = Pbody[i];
    }
   
    //  double M_t = 2.600473 * pow(10,45)/Pall.UnitMass;
    // gprintlmpi(M_t);
    //  double f_b = 0.15;
     //double epsilon = 0.7 * PC_CGS/Pall.UnitLength;
   //  double epsilon = 65.0 * PC_CGS/Pall.UnitLength;
   // double epsilon = 12.5 * PC_CGS/Pall.UnitLength;
  //  double epsilon = 10.0 * PC_CGS/Pall.UnitLength;
//   double epsilon = 7.0 * PC_CGS/Pall.UnitLength;
    double epsilon = 5.5 * PC_CGS/Pall.UnitLength;
     //double epsilon = 4.4 * PC_CGS/Pall.UnitLength;
    // double epsilon = 3.5 * PC_CGS/Pall.UnitLength;
     //  double epsilon_h = 150 * PC_CGS/Pall.UnitLength;
    ...
    for(i=0;i<Pall.Ntotal;i++){
       ...
       Pbody[i]->Use = ON;
        if(i < Pall.Nhydro){
        Pbody[i]->Type = TypeHydro;
	//	Pbody[i]->Mass = f_b*M_t/Number1;
	}else{
	  Pbody[i]->Type = TypeDM;
	  // Pbody[i]->Mass = (1.0-f_b)*M_t/Number1;
	}
        Pbody[i]->GlobalID = (i*NProcs+MyID);

	Pbody[i]->Pos[0] = Pos[i*NProcs+MyID][0];
        Pbody[i]->Pos[1] = Pos[i*NProcs+MyID][1];
	Pbody[i]->Pos[2] = Pos[i*NProcs+MyID][2];
	
	Pbody[i]->Vel[0] = Vel[i*NProcs+MyID][0];
	Pbody[i]->Vel[1] = Vel[i*NProcs+MyID][1];
	Pbody[i]->Vel[2] = Vel[i*NProcs+MyID][2];

	Pbody[i]->Mass = Mass[i*NProcs+MyID];
	...
	PbodyHydro(i)->U = Uinit;
        ...
        //CELibSetMetallicityWithSolarAbundancePattern(Pbody[mycount]->Mass,PbodyHydro(mycount)->Elements,
                // 0.1*CELibGetMetalFractionForSolarChemicalComposision());
        double MassLightElements = PbodyHydro(i)->Elements[CELibYield_H]
                                  +PbodyHydro(i)->Elements[CELibYield_He];
        double Z = (Pbody[i]->Mass-MassLightElements)/Pbody[i]->Mass;
        PbodyHydro(i)->Z = PbodyHydro(i)->ZII = PbodyHydro(i)->ZIa = Z;
#else 
        PbodyHydro(i)->Z   = 0.0;
        PbodyHydro(i)->ZII = 0.0;
        PbodyHydro(i)->ZIa = 0.0;
#endif
	}
    }
    //	printf("%e\n", Pbody[0]->Pos[0]);
  ...
  // add self-gravity
    ...
    Pall.TEnd = 13.8*GIGAYEAR_CGS/Pall.UnitTime;
    Pall.TCurrent = 0.e0;
    Pall.Redshift = 0.e0;
    Pall.Era = Pall.EraLocal = 0.e0;
    Pall.TStepTotal = 0;
    ...
    MakeDir("./data");
    strcpy(Pall.ASCIIFileName,"./data/dSphs.ASCII");
    strcpy(Pall.BaseFileName,"./data/dSphs");
    strcpy(Pall.RestartFileName,"./data/dSphs.dump");
    return ;
}
