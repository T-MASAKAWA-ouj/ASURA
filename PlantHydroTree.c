...
static void UpdateEdgesForHydroKernelMaxUpdate(void);
...
void PlantHydroTreeUpdate(void){
    ...
    //UpdateEdgesForHydroKernelMaxUpdate();
    DiagHydro = DISTANCE(HydroNode[0].Pos,HydroNode[HydroNode[0].Children].Pos);
    HydroNodeDataImplantNew(0);
    UpdateEdgesForHydro();
    TimingResults.HydroTreeThisStep += GetElapsedTime()-TimingResultThisRoutine;
    ...
    if(mode == 0){
        ...
        for(int i=0;i<NProcs-1;i++){
            MPI_Isend(iRecvAC+12*MyID,12,MPI_DOUBLE,
                    CommunicationTable[i].SendRank,
                    //TAG_PLANTTREE_EXTENSITY,MPI_COMM_WORLD,mpi_request_Export_SendAC+i);
            ...
            MPI_Irecv(iRecvAC+12*CommunicationTable[i].RecvRank,12,MPI_DOUBLE,
                    CommunicationTable[i].RecvRank,
                    //TAG_PLANTTREE_EXTENSITY,MPI_COMM_WORLD,mpi_request_Export_RecvAC+i);
...
static void UpdateEdgesForHydroKernelMaxUpdate(void){
    ...
    if(Pall.Nhydro == 0){
        ...
        for(int k=0;k<3;k++){
            maxp[k] = NBCache[0].Pos[k]+2.0*NBCache[0].Kernel;
            minp[k] = NBCache[0].Pos[k]-2.0*NBCache[0].Kernel;
        }
        for(int i=1;i<Pall.Nhydro;i++){
            for(int k=0;k<3;k++){
                maxp[k] = fmax(NBCache[i].Pos[k]+2.0*NBCache[i].Kernel,maxp[k]);
                minp[k] = fmin(NBCache[i].Pos[k]-2.0*NBCache[i].Kernel,minp[k]);
            }
        }
        TempEdges[0].PosMax[0] = maxp[0]; TempEdges[0].PosMin[0] = minp[0];
        TempEdges[0].PosMax[1] = maxp[1]; TempEdges[0].PosMin[1] = minp[1];
        TempEdges[0].PosMax[2] = maxp[2]; TempEdges[0].PosMin[2] = minp[2];
        ...
        if(Pall.NActivesHydro > 0){
            ...
            for(int i=0;i<Pall.Nhydro;i++){
                if(NBCache[i].Active){
                    if(first == true){
                        for(int k=0;k<3;k++){
                            maxp[k] = NBCache[i].Pos[k]+2.0*NBCache[i].Kernel;
                            minp[k] = NBCache[i].Pos[k]-2.0*NBCache[i].Kernel;
                        }
                        first = false; 
                    } else {
                        for(int k=0;k<3;k++){
            ...
            for(int k=0;k<3;k++)
                maxp[k] = minp[k] = HydroNode[0].Pos[k];
        }
        TempEdges[1].PosMax[0] = maxp[0]; TempEdges[1].PosMin[0] = minp[0];
        TempEdges[1].PosMax[1] = maxp[1]; TempEdges[1].PosMin[1] = minp[1];
        TempEdges[1].PosMax[2] = maxp[2]; TempEdges[1].PosMin[2] = minp[2];
    }
